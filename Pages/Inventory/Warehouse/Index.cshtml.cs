    ...
    public class IndexModel : PageModel
    {
        ...
        public IActionResult OnGet()
        {
            ...
            ViewData["LocationId"] = new SelectList(_context.InventoryLocations
                .Include(l => l.Office)
                .Where(l => l.IsActive && l.Id==1),//Make sure it's locked to Warehouse (1)
                    "Id",
                    "Name"
                );
            return Page();
        ...
        public async Task<JsonResult> OnGetMain(int locationId)
        {
            var query = _context.InventoryStocks
                ...
                .Include(i => i.Product)
                .Where(m => m.Product.ProductClass == "Main" && m.Location.Id == locationId); //1 is Warehouse, should not be changed on the DB
        ...
        public async Task<JsonResult> OnGetCollateral(int? locationId)
        {
            var query = _context.InventoryStocks
                .Include(i => i.Location)
                .Include(i => i.Product)
                .Where(m => m.Product.ProductClass == "Collateral" && m.Location.Id == 1);
            ...
            return new JsonResult(new { data = new List<InventoryStockDto>() });
        }
